...
Household with Cobb-Douglass (CD) perferences over housing $H$ and amenities $C$, CD preferences across amenity sector and CES preferences over varieties within a sector:
...
- Total housing stock (measured in units of floor space) $H_{jt}$ is assumed to be inelastic in the short-rn and follows an exogenously determined path over time.
...
## Economic model 3: Housing demand

#### Housing demand for locals

$$H_{jt}^{LT,D} (\gamma_t, a_t) = \sum_{k=1}^K M_{jt}^k (r_t, a_t) f_{jt}^k.$$

#### Housing demand for tourists

$\exists$ an exogenous number of tourists $M_t^T$ arriving into the city and choosing to stay in a short-term rental or a hotel. Housing demand from tourists in each location becomes: 

$$H_{jt}^{ST,D} (p_{jt}, a_t) = \overbrace{M_{jt}^{ST} (p_t, a_t)}^{\text{tourists staying in ST}} \cdot \underbrace{f_{jt}}_{\text{average size of a unit in location j}}$$
